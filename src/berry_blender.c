...
static void PrintPlayerNames(void)
{
    ...
    for (i = 0; i < BLENDER_MAX_PLAYERS; i++)
    {
        if (sBerryBlender->arrowIdToPlayerId[i] != NO_PLAYER)
        {
            ...
            xPos = GetStringCenterAlignXOffset(gSaveBlock2Ptr->optionsCurrentFont, text, 0x38);

            if (playerId == sBerryBlender->arrowIdToPlayerId[i])
                Blender_AddTextPrinter(i, text, xPos, 0, 0, 2); // Highlight player's name in red
            else
                Blender_AddTextPrinter(i, text, xPos, 0, 0, 1);
...
static void CB2_CheckPlayAgainLink(void)
{
    ...
    case 4:
        ...
        break;
    case 5:
        PrintMessage(&sBerryBlender->textState, gText_SavingDontTurnOff_Chat, 0);
        SetLinkStandbyCallback();
        sBerryBlender->gameEndState++;
        break;
    case 6:
    ...
    case 3:
        {
            ...
            xPos = GetStringCenterAlignXOffset(gSaveBlock2Ptr->optionsCurrentFont, sText_BlendingResults, 0xA8);
            Blender_AddTextPrinter(WIN_RESULTS, sText_BlendingResults, xPos, 0, TEXT_SKIP_DRAW, 0);
            ...
            for (i = 0; i < sBerryBlender->numPlayers; yPos += 16, i++)
            {
                ...
                MakeEReaderNameLowercase(sBerryBlender->blendedBerries[place].name);
            ...
            Blender_AddTextPrinter(WIN_RESULTS, sText_MaximumSpeed, 0, 0x50, TEXT_SKIP_DRAW, 3);
            ...
            xPos = GetStringRightAlignXOffset(gSaveBlock2Ptr->optionsCurrentFont, sBerryBlender->stringVar, 0xA8);
            Blender_AddTextPrinter(WIN_RESULTS, sBerryBlender->stringVar, xPos, 0x50, TEXT_SKIP_DRAW, 3);
            Blender_AddTextPrinter(WIN_RESULTS, sText_Time, 0, 0x60, TEXT_SKIP_DRAW, 3);
            ...
            xPos = GetStringRightAlignXOffset(gSaveBlock2Ptr->optionsCurrentFont, sBerryBlender->stringVar, 0xA8);
            Blender_AddTextPrinter(WIN_RESULTS, sBerryBlender->stringVar, xPos, 0x60, TEXT_SKIP_DRAW, 3);
...
static bool8 PrintBlendingRanking(void)
{
    ...
    case 3:
        ...
        xPos = GetStringCenterAlignXOffset(gSaveBlock2Ptr->optionsCurrentFont, sText_Ranking, 168);
        Blender_AddTextPrinter(WIN_RESULTS, sText_Ranking, xPos, 0, TEXT_SKIP_DRAW, 0);
        ...
        for (yPos = 40, i = 0; i < sBerryBlender->numPlayers; yPos += 16, i++)
        {
            u8 place = sBerryBlender->playerPlaces[i];
...
void ShowBerryBlenderRecordWindow(void)
{
    ...
    for (i = 0, yPos = 41; i < NUM_SCORE_TYPES; i++)
    {
        ...
        xPos = GetStringRightAlignXOffset(gSaveBlock2Ptr->optionsCurrentFont, text, 140);
        AddTextPrinterParameterized(gRecordsWindowId, gSaveBlock2Ptr->optionsCurrentFont, text, xPos, yPos + (i * 16), 0, NULL);
    }
...
static void Blender_AddTextPrinter(u8 windowId, const u8 *string, u8 x, u8 y, s32 speed, s32 caseId)
{
    ...
    AddTextPrinterParameterized4(windowId, gSaveBlock2Ptr->optionsCurrentFont, x, y, letterSpacing, 1, txtColor, speed, string);
}
...
static bool32 PrintMessage(s16 *textState, const u8 *string, s32 textSpeed)
{
    ...
    case 0:
        ...
        Blender_AddTextPrinter(WIN_MSG, string, 0, 0, textSpeed, 0);
