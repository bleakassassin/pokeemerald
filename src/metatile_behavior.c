...
static const u8 sTileBitAttributes[NUM_METATILE_BEHAVIORS] =
{
    ...
    [MB_CAVE_SAND]                       = TILE_FLAG_UNUSED | TILE_FLAG_HAS_ENCOUNTERS,
    ...
    [MB_FEEBAS_WATER]                    = TILE_FLAG_UNUSED | TILE_FLAG_SURFABLE | TILE_FLAG_HAS_ENCOUNTERS,
...
{
    if (metatileBehavior == MB_SAND || metatileBehavior == MB_DEEP_SAND || metatileBehavior == MB_CAVE_SAND)
        return TRUE;
    else
        return FALSE;
}
...
bool8 MetatileBehavior_IsDeepOrOceanWater(u8 metatileBehavior)
{
    if (metatileBehavior == MB_OCEAN_WATER
     ...
     || metatileBehavior == MB_DEEP_WATER
     || metatileBehavior == MB_FEEBAS_WATER)
        return TRUE;
    else
        return FALSE;
}
...
bool8 MetatileBehavior_IsFeebasWater(u8 metatileBehavior)
{
    if (metatileBehavior == MB_FEEBAS_WATER)
        return TRUE;
    else
        return FALSE;
}
