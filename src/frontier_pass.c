...
static const u16 sHeads_Pal[]                    = INCGFX_U16("graphics/frontier_pass/map_heads.png", ".gbapal");
...
static void ShowAndPrintWindows(void)
{
    ...
    x = GetStringCenterAlignXOffset(gSaveBlock2Ptr->optionsCurrentFont, gText_SymbolsEarned, 96);
    AddTextPrinterParameterized3(WINDOW_EARNED_SYMBOLS, gSaveBlock2Ptr->optionsCurrentFont, x, 4, sTextColors[0], 0, gText_SymbolsEarned);

    x = GetStringCenterAlignXOffset(gSaveBlock2Ptr->optionsCurrentFont, gText_BattleRecord, 96);
    AddTextPrinterParameterized3(WINDOW_BATTLE_RECORD, gSaveBlock2Ptr->optionsCurrentFont, x, 4, sTextColors[0], 0, gText_BattleRecord);
...
static void PrintAreaDescription(u8 cursorArea)
{
    ...
    if (cursorArea == CURSOR_AREA_RECORD && !sPassData->hasBattleRecord)
        AddTextPrinterParameterized3(WINDOW_DESCRIPTION, gSaveBlock2Ptr->optionsCurrentFont, 2, 0, sTextColors[1], 0, sPassAreaDescriptions[CURSOR_AREA_NOTHING]);
    else if (cursorArea != CURSOR_AREA_NOTHING)
        AddTextPrinterParameterized3(WINDOW_DESCRIPTION, gSaveBlock2Ptr->optionsCurrentFont, 2, 0, sTextColors[1], 0, sPassAreaDescriptions[cursorArea]);
...
static void PrintOnFrontierMap(void)
{
    ...
    AddTextPrinterParameterized3(MAP_WINDOW_DESCRIPTION, gSaveBlock2Ptr->optionsCurrentFont, 4, 0, sTextColors[0], 0, sMapLandmarks[sMapData->cursorPos].description);
