...
static void PrintCurMainMenuDescription(void)
{
    ...
    AddTextPrinterParameterized2(0, gSaveBlock2Ptr->optionsCurrentFont, sSecretBasePCMenuItemDescriptions[sDecorationActionsCursorPos], 0, 0, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE, TEXT_COLOR_LIGHT_GRAY);
}
...
static void PrintDecorationCategoryMenuItem(u8 winid, u8 category, u8 x, u8 y, bool8 disabled, u8 speed)
{
    ...
    AddTextPrinterParameterized(winid, gSaveBlock2Ptr->optionsCurrentFont, gStringVar4, x, y, speed, NULL);
    ...
    *(str++) = CHAR_SLASH;
    ...
    x = GetStringRightAlignXOffset(gSaveBlock2Ptr->optionsCurrentFont, gStringVar4, width);
    AddTextPrinterParameterized(winid, gSaveBlock2Ptr->optionsCurrentFont, gStringVar4, x, y, speed, NULL);
}
...
static u8 gpu_pal_decompress_alloc_tag_and_upload(struct PlaceDecorationGraphicsDataBuffer *data, u8 decor)
{
    ...
    if (data->decoration->shape == DECORSHAPE_2x2C || data->decoration->shape == DECORSHAPE_1x2C || data->decoration->shape == DECORSHAPE_3x3C)
        CopyPalette(data->palette, gTilesetPointer_SecretBaseRedCave->metatiles[(data->decoration->tiles[0] * NUM_TILES_PER_METATILE) + 11] >> 12);
    else
...
static u8 AddDecorationIconObjectFromObjectEvent(u16 tilesTag, u16 paletteTag, u8 decor)
{
    ...
    if (sPlaceDecorationGraphicsDataBuffer.decoration->permission != DECORPERM_SPRITE)
    {
        ...
        if (sPlaceDecorationGraphicsDataBuffer.decoration->shape == DECORSHAPE_2x2C || sPlaceDecorationGraphicsDataBuffer.decoration->shape == DECORSHAPE_1x2C || sPlaceDecorationGraphicsDataBuffer.decoration->shape == DECORSHAPE_3x3C)
            CopyPalette(sPlaceDecorationGraphicsDataBuffer.palette, gTilesetPointer_SecretBaseRedCave->metatiles[(sPlaceDecorationGraphicsDataBuffer.decoration->tiles[0] * NUM_TILES_PER_METATILE) + 11] >> 12);
        else
...
static void SetDecorRearrangementShape(u8 decor, struct DecorRearrangementDataBuffer *data)
{
    ...
    else if (gDecorations[decor].shape == DECORSHAPE_2x2 || gDecorations[decor].shape == DECORSHAPE_2x2C)
    {
        data->width = 2;
        data->height = 2;
    }
    else if (gDecorations[decor].shape == DECORSHAPE_1x2 || gDecorations[decor].shape == DECORSHAPE_1x2C)
    {
        data->width = 1;
        data->height = 2;
    }
    ...
    else if (gDecorations[decor].shape == DECORSHAPE_3x3 || gDecorations[decor].shape == DECORSHAPE_3x3C)
    {
        data->width = 3;
        data->height = 3;
    }
