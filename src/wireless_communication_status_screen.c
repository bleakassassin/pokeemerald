...
static void PrintHeaderTexts(void)
{
    ...
    WCSS_AddTextPrinterParameterized(0, gSaveBlock2Ptr->optionsCurrentFont, sHeaderTexts[0], GetStringCenterAlignXOffset(gSaveBlock2Ptr->optionsCurrentFont, sHeaderTexts[0], 0xC0), 6, COLORMODE_GREEN);
    for (i = 0; i < (int)ARRAY_COUNT(*sHeaderTexts) - 1; i++)
    {
        WCSS_AddTextPrinterParameterized(1, gSaveBlock2Ptr->optionsCurrentFont, sHeaderTexts[i + 1], 0, 30 * i + 8, COLORMODE_WHITE_LGRAY);
    }
    WCSS_AddTextPrinterParameterized(1, gSaveBlock2Ptr->optionsCurrentFont, sHeaderTexts[i + 1], 0, 30 * i + 8, COLORMODE_RED);
    PutWindowTilemap(0);
        ...
        if (UpdateCommunicationCounts(sStatusScreen->groupCounts, sStatusScreen->prevGroupCounts, sStatusScreen->activities, sStatusScreen->rfuTaskId))
        {
            ...
            for (i = 0; i < NUM_GROUPTYPES; i++)
            {
                ...
                if (i != GROUPTYPE_TOTAL)
                    WCSS_AddTextPrinterParameterized(2, gSaveBlock2Ptr->optionsCurrentFont, gStringVar4, 12, 30 * i + 8, COLORMODE_WHITE_LGRAY);
                else
                    WCSS_AddTextPrinterParameterized(2, gSaveBlock2Ptr->optionsCurrentFont, gStringVar4, 12, 98, COLORMODE_RED);
            }
            PutWindowTilemap(2);
