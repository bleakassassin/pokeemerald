...
void DrawMainBattleBackground(void)
{
    ...
    else
    {
        if (gBattleTypeFlags & BATTLE_TYPE_TRAINER)
        {
            ...
            if (trainerClass == TRAINER_CLASS_LEADER || trainerClass == TRAINER_CLASS_GYM_LEADERS)
            {
                LZDecompressVram(gBattleEnvironmentTiles_Building, (void *)(BG_CHAR_ADDR(2)));
                LZDecompressVram(gBattleEnvironmentTilemap_Building, (void *)(BG_SCREEN_ADDR(26)));
            ...
            else if (trainerClass == TRAINER_CLASS_CHAMPION)
            {
                ...
                if (gTrainerBattleOpponent_A == TRAINER_STEVEN_CHAMPION)
                    LoadCompressedPalette(gBattleEnvironmentPalette_StadiumSteven, BG_PLTT_ID(2), 3 * PLTT_SIZE_4BPP);
                else
