...
static void PrintItemDescription(int itemIndex)
{
    ...
    BagMenu_Print(WIN_DESCRIPTION, gSaveBlock2Ptr->optionsCurrentFont, str, 3, 0, 0, 0, 0, COLORID_NORMAL);
}
...
{
    ConvertIntToDecimalStringN(gStringVar1, quantity, STR_CONV_MODE_RIGHT_ALIGN, BAG_ITEM_CAPACITY_DIGITS);
    StringExpandPlaceholders(gStringVar4, gText_xVar1);
    AddTextPrinterParameterized(windowId, gSaveBlock2Ptr->optionsCurrentFont, gStringVar4, GetStringCenterAlignXOffset(gSaveBlock2Ptr->optionsCurrentFont, gStringVar4, 0x28), 0, 0, 0);
}
...
{
    ConvertIntToDecimalStringN(gStringVar1, numSold, STR_CONV_MODE_RIGHT_ALIGN, BAG_ITEM_CAPACITY_DIGITS);
    StringExpandPlaceholders(gStringVar4, gText_xVar1);
    AddTextPrinterParameterized(windowId, gSaveBlock2Ptr->optionsCurrentFont, gStringVar4, 0, 1, TEXT_SKIP_DRAW, 0);
...
static void Task_BagMenu_HandleInput(u8 taskId)
{
    ...
    if (MenuHelpers_ShouldWaitForLinkRecv() != TRUE && !gPaletteFade.active)
    {
        ...
        default:
            ...
            else if (JOY_NEW(START_BUTTON) && (gBagMenu->numItemStacks[gBagPosition.pocket] - 1) > 1
             && *cursorPos != gBagMenu->numShownItems[gBagPosition.pocket] - 1)
            {                
...
static void StartItemSwap(u8 taskId)
{
    ...
    FillWindowPixelBuffer(WIN_DESCRIPTION, PIXEL_FILL(0));
    BagMenu_Print(WIN_DESCRIPTION, gSaveBlock2Ptr->optionsCurrentFont, gStringVar4, 3, 0, 0, 0, 0, COLORID_NORMAL);
...
{
    PrintMenuActionGrid(windowId, FONT_NARROW, 8, 0, 56, columns, rows, sItemMenuActions, gBagMenu->contextMenuItemsPtr);
...
static void ItemMenu_Give(u8 taskId)
{
    ...
    {
        DisplayItemMessage(taskId, gSaveBlock2Ptr->optionsCurrentFont, gText_CantWriteMail, HandleErrorMessage);
    }
...
{
    DisplayItemMessage(taskId, gSaveBlock2Ptr->optionsCurrentFont, gText_NoPokemon, HandleErrorMessage);
}
...
static void PrintItemCantBeHeld(u8 taskId)
{
    ...
    DisplayItemMessage(taskId, gSaveBlock2Ptr->optionsCurrentFont, gStringVar4, HandleErrorMessage);
}
...
static void Task_ItemContext_Sell(u8 taskId)
{
    ...
    if (ItemId_GetPrice(gSpecialVar_ItemId) == 0 || ItemId_GetPocket(gSpecialVar_ItemId) == POCKET_TM_HM)
    {
        ...
        DisplayItemMessage(taskId, gSaveBlock2Ptr->optionsCurrentFont, gStringVar4, CloseItemMessage);
    }
    else
    {
        tItemCount = 1;
        ...
        else
        {
            ...
            DisplayItemMessage(taskId, gSaveBlock2Ptr->optionsCurrentFont, gStringVar4, InitSellHowManyInput);
        }
    }
}
...
static void DisplaySellItemPriceAndConfirm(u8 taskId)
{
    ...
    DisplayItemMessage(taskId, gSaveBlock2Ptr->optionsCurrentFont, gStringVar4, AskSellItems);
}
...
static void ConfirmSell(u8 taskId)
{
    ...
    DisplayItemMessage(taskId, gSaveBlock2Ptr->optionsCurrentFont, gStringVar4, SellItem);
}
...
static void TryDepositItem(u8 taskId)
{
    ...
    if (ItemId_GetImportance(gSpecialVar_ItemId))
    {
        ...
        BagMenu_Print(WIN_DESCRIPTION, gSaveBlock2Ptr->optionsCurrentFont, gText_CantStoreImportantItems, 3, 0, 0, 0, 0, COLORID_NORMAL);
    ...
    {
        // No room to deposit
        BagMenu_Print(WIN_DESCRIPTION, gSaveBlock2Ptr->optionsCurrentFont, gText_NoRoomForItems, 3, 0, 0, 0, 0, COLORID_NORMAL);
        gTasks[taskId].func = WaitDepositErrorMessage;
    }
}
...
static void PrepareBagForWallyTutorial(void)
{
    ...
    ClearItemSlots(gSaveBlock1Ptr->bagPocket_Medicine, BAG_ITEMS_COUNT);
...
static void PrintPocketNames(const u8 *pocketName1, const u8 *pocketName2)
{
    ...
    offset = GetStringCenterAlignXOffset(gSaveBlock2Ptr->optionsCurrentFont, pocketName1, 0x48);
    BagMenu_Print(windowId, gSaveBlock2Ptr->optionsCurrentFont, pocketName1, offset, 0, 0, 0, TEXT_SKIP_DRAW, COLORID_POCKET_NAME);
    if (pocketName2)
    {
        offset = GetStringCenterAlignXOffset(gSaveBlock2Ptr->optionsCurrentFont, pocketName2, 0x48);
        BagMenu_Print(windowId, gSaveBlock2Ptr->optionsCurrentFont, pocketName2, offset + 0x48, 0, 0, 0, TEXT_SKIP_DRAW, COLORID_POCKET_NAME);
    }
...
static void PrintTMHMMoveData(u16 itemId)
{
    ...
    {
        for (i = 0; i < 4; i++)
            BagMenu_Print(WIN_TMHM_INFO, gSaveBlock2Ptr->optionsCurrentFont, gText_ThreeDashes, 7, i * 12, 0, 0, TEXT_SKIP_DRAW, COLORID_TMHM_INFO);
    ...
    else
    {
        ...
        BagMenu_Print(WIN_TMHM_INFO, gSaveBlock2Ptr->optionsCurrentFont, text, 7, 12, 0, 0, TEXT_SKIP_DRAW, COLORID_TMHM_INFO);
        ...
        BagMenu_Print(WIN_TMHM_INFO, gSaveBlock2Ptr->optionsCurrentFont, text, 7, 24, 0, 0, TEXT_SKIP_DRAW, COLORID_TMHM_INFO);

        // Print TMHM pp
        ConvertIntToDecimalStringN(gStringVar1, gBattleMoves[moveId].pp, STR_CONV_MODE_RIGHT_ALIGN, 3);
        BagMenu_Print(WIN_TMHM_INFO, gSaveBlock2Ptr->optionsCurrentFont, gStringVar1, 7, 36, 0, 0, TEXT_SKIP_DRAW, COLORID_TMHM_INFO);
...
static void AddBagSortSubMenu(void)
{
    ...
    BagMenu_Print(1, gSaveBlock2Ptr->optionsCurrentFont, gStringVar4, 3, 0, 0, 0, 0, 0);
...
static void SortBagItems(u8 taskId)
{
    ...
    DisplayItemMessage(taskId, gSaveBlock2Ptr->optionsCurrentFont, gStringVar4, Task_SortFinish);
}
...
static void SortItemsInBag(u8 pocket, u8 type)
{
    ...
    case TREASURES_POCKET:
        ...
        break;
    case MAIL_POCKET:
        itemMem = gSaveBlock1Ptr->bagPocket_Mail;
        itemAmount = BAG_MAIL_COUNT;
        break;
    default:
        return;
    }
...
static s8 CompareItemsByType(struct ItemSlot* itemSlot1, struct ItemSlot* itemSlot2)
{
    ...
    return CompareItemsByNumber(itemSlot1, itemSlot2); //Sort by index number amongst similar types
}
