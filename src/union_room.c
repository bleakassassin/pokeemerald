...
static u8 CreateTradeBoardWindow(const struct WindowTemplate * template)
{
    ...
    PrintUnionRoomText(windowId, gSaveBlock2Ptr->optionsCurrentFont, sText_NameWantedOfferLv, 8, 1, UR_COLOR_TRADE_BOARD_OTHER);
    CopyWindowToVram(windowId, COPYWIN_GFX);
...
static void PrintGroupMemberOnWindow(u8 windowId, u8 x, u8 y, struct RfuPlayer *player, u8 colorIdx, u8 id)
{
    ...
    PrintUnionRoomText(windowId, gSaveBlock2Ptr->optionsCurrentFont, gStringVar4, x, y, UR_COLOR_DEFAULT);
    x += 18;
    ...
    if (player->groupScheduledAnim == UNION_ROOM_SPAWN_IN && !(activity & IN_UNION_ROOM))
    {
        ...
        PrintUnionRoomText(windowId, gSaveBlock2Ptr->optionsCurrentFont, gStringVar4, x, y, colorIdx);
        ...
        PrintUnionRoomText(windowId, gSaveBlock2Ptr->optionsCurrentFont, gStringVar4, GetStringRightAlignXOffset(gSaveBlock2Ptr->optionsCurrentFont, gStringVar4, 0x88), y, colorIdx);
    }
}
...
static void PrintGroupCandidateOnWindow(u8 windowId, u8 x, u8 y, struct RfuPlayer *player, u8 colorIdx, u8 id)
{
    ...
    if (player->groupScheduledAnim == UNION_ROOM_SPAWN_IN)
    {
        ...
        StringCopy(gStringVar4, sText_ID);
        StringAppend(gStringVar4, trainerId);
        PrintUnionRoomText(windowId, gSaveBlock2Ptr->optionsCurrentFont, gStringVar4, GetStringRightAlignXOffset(gSaveBlock2Ptr->optionsCurrentFont, gStringVar4, 0x68), y, colorIdx);
    }
}
...
static void TradeBoardPrintItemInfo(u8 windowId, u8 y, struct RfuGameData * data, const u8 * playerName, u8 colorIdx)
{
    ...
    PrintUnionRoomText(windowId, gSaveBlock2Ptr->optionsCurrentFont, playerName, 8, y, colorIdx);
    if (species == SPECIES_EGG)
    {
        PrintUnionRoomText(windowId, gSaveBlock2Ptr->optionsCurrentFont, sText_EggTrade, 68, y, colorIdx);
    }
    else
    {
        BlitMenuInfoIcon(windowId, type + 1, 68, y);
        PrintUnionRoomText(windowId, gSaveBlock2Ptr->optionsCurrentFont, gSpeciesNames[species], 118, y, colorIdx);
        ConvertIntToDecimalStringN(levelStr, level, STR_CONV_MODE_RIGHT_ALIGN, 3);
        PrintUnionRoomText(windowId, gSaveBlock2Ptr->optionsCurrentFont, levelStr, 198, y, colorIdx);
    }
}
