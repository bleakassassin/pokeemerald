...
static u8 sDevonCorpFossilSelections[FOSSIL_SELECTION_COUNT];
...
static void CreateDevonCorpFossilMultichoice(void);
...
void ScriptMenu_DisplayPCStartupPrompt(void)
{
    ...
    AddTextPrinterParameterized2(0, gSaveBlock2Ptr->optionsCurrentFont, gText_WhichPCShouldBeAccessed, 0, NULL, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE, TEXT_COLOR_LIGHT_GRAY);
}
...
static void CreateLilycoveSSTidalMultichoice(void)
{
    ...
    if (CheckBagHasItem(ITEM_AURORA_TICKET, 1) == TRUE && FlagGet(FLAG_RECEIVED_AURORA_TICKET) == TRUE)
    {
        if (gSpecialVar_0x8004 == 0)
        {
    ...
    if (CheckBagHasItem(ITEM_OLD_SEA_MAP, 1) == TRUE && FlagGet(FLAG_RECEIVED_OLD_SEA_MAP) == TRUE)
    {
        if (gSpecialVar_0x8004 == 0)
        {
...
bool8 ScriptMenu_CreateDevonCorpFossilMultichoice(void)
{
    if (FuncIsActiveTask(Task_HandleMultichoiceInput) == TRUE)
    {
        return FALSE;
    ...
    {
        gSpecialVar_Result = 0xFF;
        CreateDevonCorpFossilMultichoice();
        return TRUE;
    }
}
...
static void CreateDevonCorpFossilMultichoice(void)
{
    ...
    {
        sDevonCorpFossilSelections[selectionCount] = FOSSIL_SELECTION_HELIX_FOSSIL;
        selectionCount++;
    }

    if (CheckBagHasItem(ITEM_DOME_FOSSIL, 1) == TRUE)
    {
        sDevonCorpFossilSelections[selectionCount] = FOSSIL_SELECTION_DOME_FOSSIL;
        selectionCount++;
    }

    if (CheckBagHasItem(ITEM_OLD_AMBER, 1) == TRUE)
    {
        sDevonCorpFossilSelections[selectionCount] = FOSSIL_SELECTION_OLD_AMBER;
        selectionCount++;
    }

    if (CheckBagHasItem(ITEM_ROOT_FOSSIL, 1) == TRUE)
    {
        sDevonCorpFossilSelections[selectionCount] = FOSSIL_SELECTION_ROOT_FOSSIL;
        selectionCount++;
    }

    if (CheckBagHasItem(ITEM_CLAW_FOSSIL, 1) == TRUE)
    {
        sDevonCorpFossilSelections[selectionCount] = FOSSIL_SELECTION_CLAW_FOSSIL;
        selectionCount++;
    }

    sDevonCorpFossilSelections[selectionCount] = FOSSIL_SELECTION_EXIT;
    selectionCount++;
    ...
    count = selectionCount;
    {
        ...
        {
            u8 selection = sDevonCorpFossilSelections[j];
            if (selection != 0xFF)
            {
                pixelWidth = DisplayTextAndGetWidth(sDevonCorpFossils[selection], pixelWidth);
            }
        }
...
void GetDevonCorpFossilSelection(void)
{
    if (gSpecialVar_Result != MULTI_B_PRESSED)
    {
        gSpecialVar_Result = sDevonCorpFossilSelections[gSpecialVar_Result];
    }
}
...
static void DrawLinkServicesMultichoiceMenu(u8 multichoiceId)
{
    ...
    case MULTI_WIRELESS_NO_BERRY:
        ...
        AddTextPrinterParameterized2(0, gSaveBlock2Ptr->optionsCurrentFont, sWirelessOptionsNoBerryCrush[Menu_GetCursorPos()], 0, NULL, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE, TEXT_COLOR_LIGHT_GRAY);
        break;
    case MULTI_CABLE_CLUB_WITH_RECORD_MIX:
        FillWindowPixelBuffer(0, PIXEL_FILL(1));
        AddTextPrinterParameterized2(0, gSaveBlock2Ptr->optionsCurrentFont, sCableClubOptions_WithRecordMix[Menu_GetCursorPos()], 0, NULL, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE, TEXT_COLOR_LIGHT_GRAY);
        break;
    case MULTI_WIRELESS_NO_RECORD:
        FillWindowPixelBuffer(0, PIXEL_FILL(1));
        AddTextPrinterParameterized2(0, gSaveBlock2Ptr->optionsCurrentFont, sWirelessOptions_NoRecordMix[Menu_GetCursorPos()], 0, NULL, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE, TEXT_COLOR_LIGHT_GRAY);
        break;
    case MULTI_WIRELESS_ALL_SERVICES:
        FillWindowPixelBuffer(0, PIXEL_FILL(1));
        AddTextPrinterParameterized2(0, gSaveBlock2Ptr->optionsCurrentFont, sWirelessOptions_AllServices[Menu_GetCursorPos()], 0, NULL, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE, TEXT_COLOR_LIGHT_GRAY);
        break;
    case MULTI_WIRELESS_NO_RECORD_BERRY:
        FillWindowPixelBuffer(0, PIXEL_FILL(1));
        AddTextPrinterParameterized2(0, gSaveBlock2Ptr->optionsCurrentFont, sWirelessOptions_NoRecordMixBerryCrush[Menu_GetCursorPos()], 0, NULL, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE, TEXT_COLOR_LIGHT_GRAY);
        break;
    case MULTI_CABLE_CLUB_NO_RECORD_MIX:
        FillWindowPixelBuffer(0, PIXEL_FILL(1));
        AddTextPrinterParameterized2(0, gSaveBlock2Ptr->optionsCurrentFont, sCableClubOptions_NoRecordMix[Menu_GetCursorPos()], 0, NULL, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE, TEXT_COLOR_LIGHT_GRAY);
        break;
    }
}
