...
void CreateAvailableDecorationsMenu(u8 taskId)
{
    ...
    s32 windowWidth = GetStringWidth(gSaveBlock2Ptr->optionsCurrentFont, gText_Exit, 0);
    s32 fiveMarksWidth = GetStringWidth(gSaveBlock2Ptr->optionsCurrentFont, gText_FiveMarks, 0);
    for (i = 0; i < NUM_TRADER_ITEMS; i++)
    {
        s32 curWidth;
        if (trader->decorations[i] > NUM_DECORATIONS)
            ...
            curWidth = GetStringWidth(gSaveBlock2Ptr->optionsCurrentFont, gDecorations[trader->decorations[i]].name, 0);
    ...
    {
        if (trader->decorations[i] > NUM_DECORATIONS)
            AddTextPrinterParameterized(tWindowId, gSaveBlock2Ptr->optionsCurrentFont, gText_FiveMarks, 8, 16 * i + 1, TEXT_SKIP_DRAW, NULL);
        else
            AddTextPrinterParameterized(tWindowId, gSaveBlock2Ptr->optionsCurrentFont, gDecorations[trader->decorations[i]].name, 8, 16 * i + 1, TEXT_SKIP_DRAW, NULL);
    }
    AddTextPrinterParameterized(tWindowId, gSaveBlock2Ptr->optionsCurrentFont, gText_Exit, 8, 16 * i + 1, TEXT_SKIP_DRAW, NULL);
