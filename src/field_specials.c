...
static EWRAM_DATA u8 sScrollableMultichoice_iconSlot = 0;
static EWRAM_DATA u8 sScrollableMultichoice_ItemSpriteId[2] = {0};
...
static void FillFrontierExchangeCornerWindowAndItemIcon(u16, u16, u8);
...
static void HideFrontierExchangeCornerItemIcon(u16, u8);
...
static void ShowFrontierExchangeCornerItemIcon(u16, u8);
...
void ShowDeptStoreElevatorFloorSelect(void)
{
    ...
    xPos = GetStringCenterAlignXOffset(gSaveBlock2Ptr->optionsCurrentFont, sDeptStoreFloorNames[gSpecialVar_0x8005], 64);
    AddTextPrinterParameterized(sTutorMoveAndElevatorWindowId, gSaveBlock2Ptr->optionsCurrentFont, sDeptStoreFloorNames[gSpecialVar_0x8005], xPos, 16, TEXT_SKIP_DRAW, NULL);
...
static const u8 *const sScrollableMultichoiceOptions[][MAX_SCROLL_MULTI_LENGTH] =
{
    ...
    [SCROLL_MULTI_BF_EXCHANGE_CORNER_DECOR_VENDOR_2R] =
    {
        gText_LaprasDoll128BP,
        gText_SnorlaxDoll128BP,
        gText_VenusaurDoll256BP,
    ...
    [SCROLL_MULTI_BF_EXCHANGE_CORNER_VITAMIN_VENDOR] =
    {
        ...
        gText_Carbos1BP,
        gText_HpUp1BP,
        gText_MachoBrace10BP,
        gText_RareCandy20BP,
        gText_AbilityCapsule50BP,
        gText_LiechiBerry50BP,
        gText_GanlonBerry50BP,
        gText_SalacBerry50BP,
        gText_PetayaBerry50BP,
        gText_ApicotBerry50BP,
        gText_Exit
    },
    [SCROLL_MULTI_BF_EXCHANGE_CORNER_VITAMIN_VENDOR_S] =
    {
        gText_Protein1BP,
        gText_Calcium1BP,
        gText_Iron1BP,
        ...
        gText_PetayaBerry50BP,
        gText_ApicotBerry50BP,
        gText_LansatBerry100BP,
        gText_Exit
    },
    [SCROLL_MULTI_BF_EXCHANGE_CORNER_VITAMIN_VENDOR_G] =
    {
        gText_Protein1BP,
        gText_Calcium1BP,
        gText_Iron1BP,
...
static void Task_ShowScrollableMultichoice(u8 taskId)
{
    ...
    FillFrontierExchangeCornerWindowAndItemIcon(task->tScrollMultiId, 0, sScrollableMultichoice_iconSlot);
...
static void InitScrollableMultichoice(void)
{
    ...
    gScrollableMultichoice_ListMenuTemplate.fontId = gSaveBlock2Ptr->optionsCurrentFont;
...
static void ScrollableMultichoice_MoveCursor(s32 itemIndex, bool8 onInit, struct ListMenu *list)
{
    ...
    if (taskId != TASK_NONE)
    {
        ...
        FillFrontierExchangeCornerWindowAndItemIcon(task->tScrollMultiId, selection, sScrollableMultichoice_iconSlot);
        HideFrontierExchangeCornerItemIcon(task->tScrollMultiId, sScrollableMultichoice_iconSlot ^ 1);
        sScrollableMultichoice_iconSlot ^= 1;
...
void UpdateBattlePointsWindow(void)
{
    ...
    x = GetStringRightAlignXOffset(gSaveBlock2Ptr->optionsCurrentFont, string, 48);
    AddTextPrinterParameterized(sBattlePointsWindowId, gSaveBlock2Ptr->optionsCurrentFont, string, x, 0, 0, NULL);
}
...
static void FillFrontierExchangeCornerWindowAndItemIcon(u16 menu, u16 selection, u8 iconSlot)
{
    #include "data/battle_frontier/battle_frontier_exchange_corner.h"

    u8 spriteId;
    u8 *spriteIdPtr = &sScrollableMultichoice_ItemSpriteId[iconSlot];
    if (menu >= SCROLL_MULTI_BF_EXCHANGE_CORNER_DECOR_VENDOR_1 && menu <= SCROLL_MULTI_BF_EXCHANGE_CORNER_HOLD_ITEM_VENDOR)
    {
        gReservedSpritePaletteCount = 5;
        ...
        case SCROLL_MULTI_BF_EXCHANGE_CORNER_DECOR_VENDOR_1:
            ...
            {
                spriteId = AddDecorationIconObject(sFrontierExchangeCorner_Decor1[selection], 33, 88, 0, iconSlot + TAG_ITEM_ICON, iconSlot + TAG_ITEM_ICON);
        ...
        case SCROLL_MULTI_BF_EXCHANGE_CORNER_DECOR_VENDOR_2:
            ...
            {
                spriteId = AddDecorationIconObject(sFrontierExchangeCorner_Decor2[selection], 33, 88, 0, iconSlot + TAG_ITEM_ICON, iconSlot + TAG_ITEM_ICON);
                if (spriteId != MAX_SPRITES)
                    *spriteIdPtr = spriteId;
            }
            break;
        case SCROLL_MULTI_BF_EXCHANGE_CORNER_DECOR_VENDOR_2R:
            ...
            {
                spriteId = AddDecorationIconObject(sFrontierExchangeCorner_Decor2Regi[selection], 33, 88, 0, iconSlot + TAG_ITEM_ICON, iconSlot + TAG_ITEM_ICON);
                if (spriteId != MAX_SPRITES)
                    *spriteIdPtr = spriteId;
            }
            break;
...
static void ShowFrontierExchangeCornerItemIcon(u16 item, u8 iconSlot)
{
    u8 spriteId;
    u8* spriteIdPtr = &sScrollableMultichoice_ItemSpriteId[iconSlot];
    ...
    {
        spriteId = AddItemIconSprite(iconSlot + TAG_ITEM_ICON, iconSlot + TAG_ITEM_ICON, item);
        if (spriteId != MAX_SPRITES)
        {
            *spriteIdPtr = spriteId;
            gSprites[*spriteIdPtr].oam.priority = 0;
            gSprites[*spriteIdPtr].x = 36;
            gSprites[*spriteIdPtr].y = 92;
        }
    }
}

static void HideFrontierExchangeCornerItemIcon(u16 menu, u8 iconSlot)
{
    u8 *spriteIdPtr = &sScrollableMultichoice_ItemSpriteId[iconSlot];
    if (*spriteIdPtr == SPRITE_NONE)
        return;
...
void ScrollableMultichoice_RedrawPersistentMenu(void)
{
    ...
    if (taskId != TASK_NONE)
    {
        ...
        AddTextPrinterParameterized(task->tWindowId, gSaveBlock2Ptr->optionsCurrentFont, gText_SelectorArrow, 0, selectedRow * 16, TEXT_SKIP_DRAW, NULL);
...
void SwapPlayersOutfit(void)
{
    struct ObjectEvent *objEvent = &gObjectEvents[gPlayerAvatar.objectEventId];
    if (gSaveBlock2Ptr->outfitId == 0)
        gSaveBlock2Ptr->outfitId = 1;
    else
