...
static void CreateApprenticeMenu(u8 menu)
{
    ...
    for (i = 0; i < count; i++)
    {
        s32 width = GetStringWidth(gSaveBlock2Ptr->optionsCurrentFont, strings[i], 0);
        if (width > pixelWidth)
            pixelWidth = width;
    }
    ...
    for (i = 0; i < count; i++)
        AddTextPrinterParameterized(windowId, gSaveBlock2Ptr->optionsCurrentFont, strings[i], 8, (i * 16) + 1, TEXT_SKIP_DRAW, NULL);
