...
static void PrintMailText(void)
{
    ...
    for (i = 0; i < sMailRead->layout->numLines; i ++)
    {
        ...
        AddTextPrinterParameterized3(0, gSaveBlock2Ptr->optionsCurrentFont, sMailRead->layout->lines[i].xOffset + sMailRead->layout->wordsXPos, y + sMailRead->layout->wordsYPos, sTextColors, 0, sMailRead->message[i]);
        y += sMailRead->layout->lines[i].height;
    }
    bufptr = StringCopy(signature, gText_FromSpace);
    StringCopy(bufptr, sMailRead->playerName);
    box_x = GetStringCenterAlignXOffset(gSaveBlock2Ptr->optionsCurrentFont, signature, sMailRead->signatureWidth) + 104;
    box_y = sMailRead->layout->signatureYPos + 88;
    AddTextPrinterParameterized3(0, gSaveBlock2Ptr->optionsCurrentFont, box_x, box_y, sTextColors, 0, signature);
