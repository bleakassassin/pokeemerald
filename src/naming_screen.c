...
static void DisplaySentToPCMessage(void)
{
    ...
    AddTextPrinterParameterized2(0, gSaveBlock2Ptr->optionsCurrentFont, gStringVar4, GetPlayerTextSpeedDelay(), 0, 2, 1, 3);
...
static void DrawNormalTextEntryBox(void)
{
    ...
    AddTextPrinterParameterized(sNamingScreen->windows[WIN_TEXT_ENTRY_BOX], gSaveBlock2Ptr->optionsCurrentFont, sNamingScreen->template->title, 8, 1, 0, 0);
...
static void DrawMonTextEntryBox(void)
{
    ...
    FillWindowPixelBuffer(sNamingScreen->windows[WIN_TEXT_ENTRY_BOX], PIXEL_FILL(1));
    AddTextPrinterParameterized(sNamingScreen->windows[WIN_TEXT_ENTRY_BOX], gSaveBlock2Ptr->optionsCurrentFont, buffer, 8, 1, 0, 0);
    PutWindowTilemap(sNamingScreen->windows[WIN_TEXT_ENTRY_BOX]);
}
...
static void DrawGenderIcon(void)
{
    ...
    if (sNamingScreen->monGender != MON_GENDERLESS)
    {
        ...
        AddTextPrinterParameterized3(sNamingScreen->windows[WIN_TEXT_ENTRY], gSaveBlock2Ptr->optionsCurrentFont, 0x68, 1, sGenderColors[isFemale], TEXT_SKIP_DRAW, text);
    }
}
...
static void DrawTextEntry(void)
{
    ...
    for (i = 0; i < maxChars; i++)
    {
        ...
        AddTextPrinterParameterized(sNamingScreen->windows[WIN_TEXT_ENTRY], gSaveBlock2Ptr->optionsCurrentFont, temp, i * 8 + x + extraWidth, 1, TEXT_SKIP_DRAW, NULL);
    }
...
static void PrintKeyboardKeys(u8 window, u8 page)
{
    ...
    for (i = 0; i < KBROW_COUNT; i++)
        AddTextPrinterParameterized3(window, gSaveBlock2Ptr->optionsCurrentFont, 0, i * 16 + 1, sKeyboardTextColors[page], 0, sNamingScreenKeyboardText[page][i]);
