...
static void BuyMenuPrintItemDescriptionAndShowItemIcon(s32 item, bool8 onInit, struct ListMenu *list)
{
    ...
    BuyMenuPrint(WIN_ITEM_DESCRIPTION, description, 3, 0, 0, COLORID_NORMAL);
}
...
static void BuyMenuPrint(u8 windowId, const u8 *text, u8 x, u8 y, s8 speed, u8 colorSet)
{
    AddTextPrinterParameterized4(windowId, gSaveBlock2Ptr->optionsCurrentFont, x, y, 0, 0, sShopBuyMenuTextColors[colorSet], speed, text);
}

static void BuyMenuDisplayMessage(u8 taskId, const u8 *text, TaskFunc callback)
{
    DisplayMessageAndContinueTask(taskId, WIN_MESSAGE, 10, 14, gSaveBlock2Ptr->optionsCurrentFont, GetPlayerTextSpeedDelay(), text, callback);
...
static void Task_BuyHowManyDialogueInit(u8 taskId)
{
    ...
    BuyMenuPrint(WIN_QUANTITY_IN_BAG, gStringVar4, 0, 0, 0, COLORID_NORMAL);
    tItemCount = 1;
...
static void Task_BuyHowManyDialogueHandleInput(u8 taskId)
{
    ...
    else
    {
        if (JOY_NEW(A_BUTTON))
        {
            ...
            ConvertIntToDecimalStringN(gStringVar2, tItemCount, STR_CONV_MODE_LEFT_ALIGN, BUY_ITEM_CAPACITY_DIGITS);
...
static void BuyMenuPrintItemQuantityAndPrice(u8 taskId)
{
    ...
    PrintMoneyAmount(WIN_QUANTITY_PRICE, 38, 0, sShopData->totalCost, TEXT_SKIP_DRAW);
    ConvertIntToDecimalStringN(gStringVar1, tItemCount, STR_CONV_MODE_LEADING_ZEROS, BUY_ITEM_CAPACITY_DIGITS);
    StringExpandPlaceholders(gStringVar4, gText_xVar1);
    BuyMenuPrint(WIN_QUANTITY_PRICE, gStringVar4, 0, 0, 0, COLORID_NORMAL);
}
