...
static s32 DrawResultsTextWindow(const u8 *text, u8 spriteId)
{
    ...
    strWidth = GetStringWidth(gSaveBlock2Ptr->optionsCurrentFont, text, 0);
    tileWidth = (strWidth + 9) / 8;
    if (tileWidth > DISPLAY_TILE_WIDTH)
        tileWidth = DISPLAY_TILE_WIDTH;

    AddTextPrinterParameterized3(windowId, gSaveBlock2Ptr->optionsCurrentFont, (tileWidth * 8 - strWidth) / 2, 0, sContestLinkTextColors, TEXT_SKIP_DRAW, text);
    {
        s32 i;
