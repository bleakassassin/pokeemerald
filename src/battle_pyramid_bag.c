...
static void PrintSelectorArrowAtPos(u8 y, u8 colorId)
{
    if (colorId == COLORID_NONE) // If 'no color', erase arrow
        FillWindowPixelRect(WIN_LIST, PIXEL_FILL(0), 0, y, GetMenuCursorDimensionByFont(gSaveBlock2Ptr->optionsCurrentFont, 0), GetMenuCursorDimensionByFont(gSaveBlock2Ptr->optionsCurrentFont, 1));
    else
...
static void ShowNumToToss(void)
{
    ...
    x = GetStringCenterAlignXOffset(gSaveBlock2Ptr->optionsCurrentFont, gStringVar4, 0x28);
    AddTextPrinterParameterized(WIN_TOSS_NUM, gSaveBlock2Ptr->optionsCurrentFont, gStringVar4, x, 2, 0, NULL);
}
...
{
    AddTextPrinterParameterized4(windowId, gSaveBlock2Ptr->optionsCurrentFont, x, y, letterSpacing, lineSpacing, sTextColors[colorTableId], speed, src);
}
...
void DisplayItemMessageInBattlePyramid(u8 taskId, const u8 *str, void (*callback)(u8 taskId))
{
    ...
    DisplayMessageAndContinueTask(taskId, WIN_MSG, 0xA, 0xD, gSaveBlock2Ptr->optionsCurrentFont, GetPlayerTextSpeedDelay(), str, callback);
    ScheduleBgCopyTilemapToVram(1);
}
