...
static u8 CanMonLearnTMTutor(struct Pokemon *mon, u16 item, u8 tutor)
{
    ...
    else if (tutor == TUTOR_ULTIMATE_MOVES)
    {
        for (i = 0; i < ARRAY_COUNT(sUltimateMoves); i++)
        {
            ...
            {
                gPartyMenu.data[0] = move = GetTutorMove(sUltimateMoves[i]);
...
static void DisplayPartyPokemonDescriptionText(u8 stringID, struct PartyMenuBox *menuBox, u8 c)
{
    ...
    if (c != 2)
        AddTextPrinterParameterized3(menuBox->windowId, gSaveBlock2Ptr->optionsCurrentFont, menuBox->infoRects->descTextLeft, menuBox->infoRects->descTextTop, sFontColorTable[0], 0, sDescriptionStringTable[stringID]);
}
...
void DisplayPartyMenuStdMessage(u32 stringId)
{
    ...
    if (stringId != PARTY_MSG_NONE)
    {
        ...
        AddTextPrinterParameterized(*windowPtr, gSaveBlock2Ptr->optionsCurrentFont, gStringVar4, 0, 0, 0, 0);
        ScheduleBgCopyTilemapToVram(2);
    }
}
...
static u8 DisplaySelectionWindow(u8 windowType)
{
    ...
    cursorDimension = GetMenuCursorDimensionByFont(gSaveBlock2Ptr->optionsCurrentFont, 0);
    letterSpacing = GetFontAttribute(gSaveBlock2Ptr->optionsCurrentFont, FONTATTR_LETTER_SPACING);

    for (i = 0; i < sPartyMenuInternal->numActions; i++)
    {
        u8 fontColorsId = (sPartyMenuInternal->actions[i] >= MENU_FIELD_MOVES) ? 4 : 3;
        AddTextPrinterParameterized4(sPartyMenuInternal->windowId[0], gSaveBlock2Ptr->optionsCurrentFont, cursorDimension, (i * 16), letterSpacing, 0, sFontColorTable[fontColorsId], 0, sCursorOptions[sPartyMenuInternal->actions[i]].text);
    }
...
static void PrintMessage(const u8 *text)
{
    ...
    AddTextPrinterParameterized2(WIN_MSG, gSaveBlock2Ptr->optionsCurrentFont, text, GetPlayerTextSpeedDelay(), 0, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE, TEXT_COLOR_LIGHT_GRAY);
}
...
static bool8 IsHPRecoveryItem(u16 item)
{
    ...
    if (item == ITEM_ENIGMA_BERRY)
        effect = gSaveBlock3Ptr->enigmaBerry.itemEffect;
    else
...
static void ShowMoveSelectWindow(u8 slot)
{
    u8 i;
    u8 moveCount = 0;
    u8 fontId = gSaveBlock2Ptr->optionsCurrentFont;
...
static void TryTutorSelectedMon(u8 taskId)
{
    ...
    if (!gPaletteFade.active)
    {
        ...
        default:
            ...
            else if (GiveMoveToMon(mon, gPartyMenu.data[0]) != MON_HAS_MAX_MOVES)
            {
                Task_LearnedMove(taskId);
                return;
            }
