...
{
    AddTextPrinterParameterized4(windowId, gSaveBlock2Ptr->optionsCurrentFont, x, y, 0, 0, sTextColors[colorStructId], speed, text);
}
...
static void AddBerryTagTextToBg0(void)
{
    ...
    PrintTextInBerryTagScreen(WIN_BERRY_TAG, gText_BerryTag, GetStringCenterAlignXOffset(gSaveBlock2Ptr->optionsCurrentFont, gText_BerryTag, 0x40), 1, 0, 1);
...
{
    const struct Berry *berry = GetBerryInfo(sBerryTag->berryId);
    AddTextPrinterParameterized(WIN_SIZE_FIRM, gSaveBlock2Ptr->optionsCurrentFont, gText_SizeSlash, 0, 1, TEXT_SKIP_DRAW, NULL);
    ...
    if (berry->size != 0)
    {
        ...
        AddTextPrinterParameterized(WIN_SIZE_FIRM, gSaveBlock2Ptr->optionsCurrentFont, gStringVar4, 0x28, 1, 0, NULL);
    }
    else
    {
        AddTextPrinterParameterized(WIN_SIZE_FIRM, gSaveBlock2Ptr->optionsCurrentFont, gText_ThreeMarks, 0x28, 1, 0, NULL);
    }
}
...
{
    const struct Berry *berry = GetBerryInfo(sBerryTag->berryId);
    AddTextPrinterParameterized(WIN_SIZE_FIRM, gSaveBlock2Ptr->optionsCurrentFont, gText_FirmSlash, 0, 0x11, TEXT_SKIP_DRAW, NULL);
    if (berry->firmness != 0)
        AddTextPrinterParameterized(WIN_SIZE_FIRM, gSaveBlock2Ptr->optionsCurrentFont, sBerryFirmnessStrings[berry->firmness - 1], 0x28, 0x11, 0, NULL);
    else
        AddTextPrinterParameterized(WIN_SIZE_FIRM, gSaveBlock2Ptr->optionsCurrentFont, gText_ThreeMarks, 0x28, 0x11, 0, NULL);
}
...
static void PrintBerryDescription1(void)
{
    ...
    {
        AddTextPrinterParameterized(WIN_DESC, gSaveBlock2Ptr->optionsCurrentFont, sBerryDescriptionPart1_WatmelMetric, 0, 1, 0, NULL);
    }
    else
    {
        const struct Berry *berry = GetBerryInfo(sBerryTag->berryId);
...
static void PrintBerryDescription2(void)
{
    ...
    if (gSaveBlock2Ptr->optionsUnitSystem == 1 && sBerryTag->berryId == ItemIdToBerryType(ITEM_WATMEL_BERRY))
    {
        AddTextPrinterParameterized(WIN_DESC, gSaveBlock2Ptr->optionsCurrentFont, sBerryDescriptionPart2_WatmelMetric, 0, 0x11, 0, NULL);
    }
    else
    {
        const struct Berry *berry = GetBerryInfo(sBerryTag->berryId);
